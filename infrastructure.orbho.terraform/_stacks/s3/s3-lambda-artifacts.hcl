...
inputs = {
  ...
  bucket_name = "dep-${local.infra_environment}-${local.aws_region}-lambda-artifacts" 
  acl = "private"
  create_object = true
  key = "lambdaauthorizer/"
  lifecycle_rule = [
    {
  ...
  kms_master_key_id = dependency.kms-lambda-artifacts.outputs.kms_arn
