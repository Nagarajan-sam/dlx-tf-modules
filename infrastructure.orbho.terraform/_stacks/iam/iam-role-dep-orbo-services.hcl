...
inputs = {
  role_name          = "${local.infra_environment}-${local.aws_region}-dep-orbo-services-role"
  role_description   = "Role used by DEP Orbo Services (${local.app_environment})"
  policy_name        = "${local.infra_environment}-${local.aws_region}-dep-orbo-services-policy"
  policy_description = "Policy used by DEP Orbo Services Role (${local.app_environment})"
  ...
  policy_statement = templatefile("policy-services/statement.tftpl", {
