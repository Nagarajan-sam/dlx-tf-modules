...
inputs = {
  create_role        = true
  create_policy      = true
  role_name          = "${local.infra_environment}-${local.aws_region}-dep-documents"
  role_description   = "Role used by s3-dep-documents (${local.app_environment})"
  policy_name        = "${local.infra_environment}-${local.aws_region}-dep-documents"
  policy_description = "Policy used by s3-dep-documents (${local.app_environment})"
  role_path          = "/"
