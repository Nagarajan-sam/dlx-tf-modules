locals {
  ...
  app_name             = "${local.infra_environment}-dep-orbo-services"
...
dependency "iam-role-dep-orbo-services" {
  config_path = "${get_terragrunt_dir()}/../iam-role-dep-orbo-services"
...
inputs = {
  ...
  iam_instance_profile = dependency.iam-role-dep-orbo-services.outputs.iam_instance_profile_id
  ...
  target_group_arns   = [element(dependency.alb-dep-services.outputs.loadbalancer_target_group_arns, 0)]
  vpc_zone_identifier = local.vpc_info.web_subnet_ids
